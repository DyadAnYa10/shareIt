...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto updateById(ItemDto itemDto, Long id, Long userId) {
        Item foundItem = checkItem(id);

        if (!Objects.equals(foundItem.getOwner().getId(), userId)) {
            throw new DeniedAccessException(String.format("Owner of item with id='%s' is another", foundItem.getId()));
        }
    ...
    public List<ItemDto> getAllByUserId(int from, int size, long userId) {
        if (from < 0 || size <= 0) {
            throw new NotFoundException("Ошибка пагинации");
        }
    ...
    public CommentDto createComment(CommentDto dto, Long itemId, Long userId) {
        if (dto.getText().isBlank()) {
            throw new NotFoundException("Ошибка: пустой комментарий");
        }
        ...
        if (!bookingRepository.existsBookingByItemAndBookerAndStatusNotAndStartBefore(item, author, BookingStatus.REJECTED, LocalDateTime.now())) {
            throw new ValidateCommentException(" Ошибка: комментарий для itemId: " + itemId);
        }
    ...
    private ItemRequest findItemRequestById(Long requestId) {
        return itemRequestRepository.findById(requestId).orElseThrow(() -> new NotFoundException(String.format(
                "Запрос c id = %s не существует", requestId))
        );
    }
